...
- Jenkins 기반 Android·iOS 빌드와 주요 앱 스토어 배포

## 주요 성과

- 서버 요청과 데이터·리소스 로드를 병렬화하여 앱 실행부터 로비 진입까지의 초기 로딩 시간을 약 20초에서 약 5초로 단축
- 반복적인 ID 조회가 필요한 데이터 테이블을 List에서 Dictionary로 전환하고 생성 템플릿과 참조 코드를 일괄 수정
- 분산된 스탯 데이터를 공통 스테이터스 테이블과 ID 참조 방식으로 재설계하여 데이터 중복과 관리 복잡도 개선
- Addressables와 PAD 기반으로 리소스 관리 방식을 일원화하고 로드·해제 상태를 추적하는 내부 관리 도구 제작
- 출시 후 약 2개월간 Logcat과 Unity 디버깅으로 크래시 개선을 주도하여 크래시율을 약 10%에서 약 2%로 감소
